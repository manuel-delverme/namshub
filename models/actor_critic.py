...
from commons.tf_utils import fc, get_trainable_variables, transfer_learning, categorical_sample
    ...
    def __build_graph(self, acts_dim, units=64, act=tf.nn.relu):
        with tf.variable_scope(self.scope):
            ...
            with tf.variable_scope('actor'):
                self.logits = fc(h, h_size=acts_dim, act=None, name='actor')
                self.pi = tf.nn.softmax(self.logits)
                self.log_pi = tf.nn.log_softmax(self.logits)
                action = categorical_sample(logits=self.logits)
                self.sample = tf.clip_by_value(action, clip_value_min=0, clip_value_max=1)
